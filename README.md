...
# Installation
バイナリ一式を任意の場所に配置すれば、各 `.exe` は直接実行できます。
...
ダウンロードしたインストーラ（setup.exe）を実行してください。

.NET 6.0 以上が必要ですが、未インストールの場合は
初回起動時に Microsoft の案内に従ってインストールできます。

### 3. 初めての作品を開く
1. **CbMesh.exe** を起動します（シリーズアプリのランチャー）
2. 公式サイトに掲載されている任意のフリーデータをダウンロードします
3. ダウンロードしたデータを開き、プレビューを確認します
4. 寸法や本数などの数値を変更して、結果の変化を確認してみてください
