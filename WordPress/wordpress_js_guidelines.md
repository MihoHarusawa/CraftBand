...
このフォルダに置いているのは、WordPressの「カスタムHTML」ブロックとして実装するためのhtml(JavaScript)コードです。ローカルに開いて動作確認するための`<ツール名>`.html と、同機能でWordPressの「カスタムHTML」ブロックに入れる`<ツール名>`-snippet.html をセットにしています。
この文書は、WordPressの「カスタムHTML」ブロックで安全かつ確実にJavaScriptツールを動作させるための経験に基づくノウハウまとめです。今後、新しいツールを追加する際のリファレンスとして活用するとともに、重要事項が見つかれば随時追加してください。
...
**例1: \&\&（かつ）を使わない書き換え**
論理積（AND）は、if文を入れ子（ネスト）にすることで同じ意味を表現できます。
...
**例2: ||（または）を使わない書き換え**
論理和（OR）は、三項演算子を使うことで安全に表現できます。特にデフォルト値の設定などで多用されます。
...
const val = !isNaN(rawVal) ? rawVal : 0;
```
...
## 9\. 特殊記号・アイコン・絵文字の「実体参照」化

WordPressの `wptexturize` などのフィルターは、記号（▶, ▼, ↶, 💡等）を勝手にHTMLエンティティ（`\\\&#25b6;` 等）に変換したり、絵文字を画像に置換したりします。これがCSSの `content` 内やJSの文字列内で起こると、表示が壊れたり「\&#25b6:」のような文字列がそのまま表示される原因になります。

**【対策コード】**

* **CSS内**: 直接記号を書かず、Unicodeエスケープシーケンス（`\\\\` + 16進数）を使います。
* **HTML/JS内**: 文字実体参照（`\\\&#x...;`）を使います。

```html
<style>
    /\\\* 【NG】 content: '▶'; -> 文字化けやエンティティ化のリスク \\\*/
    /\\\* 【OK】 Unicodeエスケープを使う \\\*/
...
<!-- 【OK】 絵文字も同様に実体参照にするのが安全 -->
<span>\\\&#x1F4A1; ヒント</span>
```

\---

## 10\. スマホ（iOS等）での `input\\\[type="color"]` の起動制限の回避

スマホのブラウザ（特にiOS Safariなど）では、セキュリティ制限により、JavaScriptから非表示の `input\\\[type="color"]` に対して `.click()` を実行しても、ネイティブのカラーピッカーが開きません。

**【対策コード】**
JavaScript で `.click()` を代行するのではなく、**対象のボタン要素（四角い色枠など）と同じサイズの `<input type="color">` を CSS の `position: absolute;` で完全に重ね合わせ、`opacity: 0` で透明化**します。これにより、ユーザーが画面をタップした際に「透明な `input` 要素を直接タップした」ことになり、すべての端末で確実にカラーピッカーが開きます。

```html
<!-- 【NG】 JSで click() を呼び出す手法（スマホで開かない） -->
<div id="swatchA" style="background: red;">A</div>
<input type="color" id="pickerA" style="display: none;">
<script>
    document.getElementById('swatchA').onclick = () => {
        document.getElementById('pickerA').click(); // スマホではブロックされる
    };
</script>

<!-- 【OK】 CSSで透明にして重ねる手法（スマホでも確実に開く） -->
<div style="position: relative; width: 50px; height: 50px;">
    <!-- 見た目の要素 -->
    <div style="background: red; position: absolute; top: 0; left: 0; width: 100%; height: 100%;">A</div>
    <!-- 透明にして重ねる input要素 -->
    <input type="color" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; opacity: 0; cursor: pointer;">
</div>
```

\---

## 11\. WordPressの予約クエリ変数（Reserved Query Variables）との衝突による404エラー対策

WordPressで構築されたサイト上でURLパラメータ（クエリパラメータ）を受け取るツールを動作させる場合、**一般的な英単語（特に `w` や `d` など）をパラメータ名としてそのまま使用すると、WordPressシステムが動作を乗っ取り「404 Not Found (ページが見つかりません)」エラーが発生**します。

これは、WordPressコアが日付やアーカイブ検索のクエリ用に特定の単語（例：`w` = Week、`d` = Day、`h` = Hourなど）を「予約されたクエリ変数（Reserved Query Vars）」として内部定義しているためです。

**【対策コード】**
WordPress上でホストするすべてのツールにおいて、URLパラメータを受け取る際は、必ず独自のプレフィックス（例：`cb_`）を付与したパラメータ名を使用します。

```javascript
// 【NG】 WordPressの予約語「w」(Week)と衝突し、ページ全体が404エラーになる！
// URL例: https://example.com/tool/?w=250
// let wTarget = parseFloat(urlParams.get('w'));

// 【OK】 プレフィックスを付与して衝突を完璧に回避
// URL例: https://example.com/tool/?cb_w=250
let wTarget = parseFloat(urlParams.get('cb_w'));
```

これにより、ドメイン名や下位フォルダのURL構成が変わっても、404エラーを起こさずに安全かつ確実に引数を読み取ることができます。

## 12\. JSによる値変更時のDOMイベント明示的トリガー（UIステッパーや計算の同期）

プログラムから直接 HTML `input` 要素の `.value` プロパティを書き換えた場合（例：URLパラメータの解析後に数値をセットする場合など）、ブラウザ仕様により `input` や `change` イベントは**自動的に発火しません**。

これにより、ツール側に実装されているステッパー処理（`input.addEventListener('input', ...)` 等で前回の値を `prevValue` としてクロージャ内に保持し、奇数偶数を制限する処理など）がある場合、**クロージャ内の状態が同期されず、その後のユーザー操作（テキストボックスの変更や上下矢印キーの押下）のタイミングで入力値や計算値に不整合が発生**します。

**【対策コード】**
プログラムから値を代入した際は、必ず直後に `.dispatchEvent(new Event('input'))` を明示的に呼び出してイベントを発火させ、UIステッパーのクロージャ値（`prevValue`等）や、関連する計算処理を自動同期させます。

```javascript
// 【NG】 値は変わるが、リスナーが動かず、その後のユーザー操作で不整合が起きる
horizontal.value = String(bestW);

// 【OK】 値の代入と同時にイベントを発火させ、UI制御と計算ロジックを確実に同期
horizontal.value = String(bestW);
horizontal.dispatchEvent(new Event('input'));
```

これにより、URL引数のロード時やプログラムからのプリセット適用時でも、その後のUI操作を含めて完全に同期されたスムーズなユーザー体験を提供できます。

\---

## まとめ

これら12個のルール（**コンテナ起点での要素取得、IIFEで変数隔離、CSSカプセル化、リスナーによるイベント付与、ライブラリロード、演算子の破壊対策、特殊記号のエスケープ、クレジット表記の埋め込み、フッターの共通仕様、スマホでのカラーピッカー起動対策、WordPress予約変数との衝突防止、値変更時のイベント明示的トリガー**）を守るだけで、安全で再利用性が高く、かつトラブルの少ないツールを作成することができます。
