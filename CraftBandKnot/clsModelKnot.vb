...
Public Class clsModelKnot
    ...
    '対象指定
    Sub New(ByVal calc As clsCalcKnot, ByVal fpath As String)
        MyBase.New(fpath)
        _calc = calc
    End Sub
    ...
    Private Function setRegions() As Boolean
        '底の横
        Dim yoko As Double = _calc.p_dコマベース_横
        '底の縦
        Dim tate As Double = _calc.p_dコマベース_縦
        '高さ
        Dim takasa As Double = _calc.p_dコマベース_高さ
    ...
    Function setDataForTexture() As Boolean
        ...
        '開始位置なし
        _data画像生成用.p_row底_縦横.Value("f_i左から何番目") = 0
    ...
    Function getImages() As Boolean
        ...
        Dim calcTmp As New clsCalcKnot(_data画像生成用, _calc._frmMain)
        ...
        If calcTmp.CalcSize(CalcCategory.NewData, Nothing, Nothing) Then
            ...
            Else
                ...
                For i As Integer = 0 To cBasketPlateCount - 1
                    ...
                    If i = 0 Then
                        '底
                        _delta画像サイズ(i) = New S差分(calcTmp.p_dコマベース_横, calcTmp.p_dコマベース_縦)
                    ElseIf {1, 3}.Contains(i) Then
                        '右側面と左側面
                        _delta画像サイズ(i) = New S差分(calcTmp.p_dコマベース_縦, calcTmp.p_dコマベース_高さ)
                    Else
                        '前面と背面
                        _delta画像サイズ(i) = New S差分(calcTmp.p_dコマベース_横, calcTmp.p_dコマベース_高さ)
                    End If
