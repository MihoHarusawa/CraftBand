    ...
    Private Function calc_側面(ByVal category As CalcCategory, ByVal row As tbl側面Row, ByVal dataPropertyName As String) As Boolean
        ...
        If _enum配置タイプ = enum配置タイプ.i_輪弧 Then
            ...
            If ary Is Nothing OrElse ary.Count < 2 Then
                _d高さの輪弧長 = 0
            Else
                _d高さの輪弧長 = New S線分(ary(0), ary(1)).Length
            End If
    ...
    Function adjust_縦ひも(ByVal row As tbl縦横展開Row, ByVal dataPropertyName As String) As Boolean
        ...
        If row.f_iひも種 = enumひも種.i_縦 Then
            If _enum配置タイプ = enum配置タイプ.i_輪弧 Then
                ...
                row.f_d長さ = _d高さの輪弧長   '高さで算出しているので、垂直ひも長分を加える
                row.f_dひも長 = _d高さの輪弧長 + 2 * ((_d垂直ひも長合計 - _d高さの合計) + _d垂直ひも長加算)
...
#Region "画面から文字列取得"
    ...
    Private Function text縦ひも_輪弧() As String
        Return _frmMain.lbl縦ひも_輪弧.Text
    End Function
