...
CbMesh.exe 経由で起動した場合、呼び出し側が取得できる終了コードは **CbMesh.exe 自身のもの** であり、個別 EXE の起動に成功した時点で正常終了（0）となります。**個別 EXE の処理結果や終了コードは CbMesh.exe 経由ではわかりません**（詳細は §9）。
...
### 9.2. CbMesh.exe 経由で起動した場合

CbMesh.exe は対応する個別 EXE を起動するランチャーです。`start /wait` 等で取得できる終了コードは **CbMesh.exe のもの** であり、上記 §9.1 の表・成否の目安は **個別 EXE に適用されません**。

* **正常終了（0）**: データの判別と、対応する個別 EXE の起動に成功した時点で CbMesh.exe は終了します。個別 EXE がその後ヘッドレス処理を完了したか、出力ファイルが生成されたかは、この終了コードからは判断できません。
* **異常終了（0 以外）**: CbMesh.exe 側でデータ判別失敗・起動失敗などが起きた場合（§9.1 の 97・98 等に相当する事象）。
* **個別 EXE の処理結果**: ヘッドレス出力の成否、§9.1 の終了コード、ログの内容は **CbMesh.exe 経由では呼び出し側に返りません**。個別 EXE の処理結果まで自動化で判定する必要がある場合は、所属 EXE が分かっているときは **対応 EXE を直接起動** してください（§1 参照）。
