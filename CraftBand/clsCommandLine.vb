...
Public Class clsCommandLine
    ...
    Public Property MasterPath As String = Nothing
    Private Property ImageOutputPath As String = Nothing
    Private Property Image2OutputPath As String = Nothing
    Private Property ListOutputPath As String = Nothing
    ...
    '警告メッセージ
    Dim _warning As New System.Text.StringBuilder
    Public ReadOnly Property Warning As String
        Get
            Return _warning.ToString()
        End Get
    End Property
    Sub AddWarning(ByVal message As String)
        If String.IsNullOrEmpty(message) Then
            Return
        End If
        _warning.AppendLine(message)
    End Sub
    ...
    Sub New(ByVal args() As String)
        ...
        For i As Integer = 0 To args.Length - 1
            ...
            If lower = "--config" Then
                ...
                If String.IsNullOrWhiteSpace(configPath) OrElse Not File.Exists(configPath) Then
                    _warning.AppendLine($"No config file '{configPath}'")
                    IsValid = False
                    IsHeadlessMode = True 'コンソール表示
                    Exit Sub
                End If
                Exit For
            End If
        ...
        If Not String.IsNullOrWhiteSpace(configPath) Then
            ...
            Catch ex As Exception
                'ファイルを指定したならNGはエラー
                _warning.AppendLine($"Failed to load config from '{configPath}': {ex.Message}")
                IsValid = False
                IsHeadlessMode = True 'コンソール表示
                Exit Sub
            End Try
        ...
        While iIndex < useArgs.Length
            ...
            Select Case key
                ...
                Case "master", "m"
                    iIndex += 1
                    If iIndex < useArgs.Length Then
                        MasterPath = useArgs(iIndex)
                    End If

                Case "image", "i"
                    iIndex += 1
                    If iIndex < useArgs.Length Then
                        ImageOutputPath = useArgs(iIndex)
                        IsHeadlessMode = True
                    End If
                ...
                Case "image2", "i2"
                    iIndex += 1
                    If iIndex < useArgs.Length Then
                        Image2OutputPath = useArgs(iIndex)
                        IsHeadlessMode = True
                    End If
                ...
                Case Else
                    ...
                    If IO.File.Exists(useArgs(iIndex)) Then
                        If 0 < _dataList.Count Then
                            _warning.AppendLine($"Multiple data files specified: '{useArgs(iIndex)}'")
                        End If
                        _dataList.Add(useArgs(iIndex))
                    Else
                        _warning.AppendLine($"Unknown argument or file not found: '{useArgs(iIndex)}'")
                        IsValid = False
                    End If
            End Select
    ...
    '*正式記述
    '{
    '  "master": "C:/App/Master/craftbandmesh_20260515.CBMESH",
    ...
    '}
    '*配列でも可（ただし順番に依存する）
    '["--data" , "C:/Users/miho/Desktop/Square-Snow.cbmesh" , "--master" , "C:/App/Master/craftbandmesh_20260515.CBMESH", "--list", "C:/App/Export/result.csv"  ]
    ...
    Private Function loadArgsFromConfigFile(ByVal path As String) As String()
        ...
        Catch ex As JsonException
            ...
            _warning.AppendFormat("Config file is not valid JSON, falling back to command-line style parsing: {0}", ex.Message).AppendLine()
        End Try
    ...
    Public Function ExecuteHeadlessMode(ByVal mainForm As Windows.Forms.Form) As Boolean
        ...
        If String.IsNullOrWhiteSpace(ImageOutputPath) AndAlso
            ...
            AddWarning("image,image2 cannot be the same path.")
            Return False
        End If
        ...
        Try
            ...
            If Not String.IsNullOrWhiteSpace(Image2OutputPath) Then
                ...
                If String.IsNullOrWhiteSpace(Path.GetExtension(img2fpath)) Then
                    ' 拡張子省略 -> treat as base name, create folder next to it
