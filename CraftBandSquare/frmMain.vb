...
Public Class frmMain
    ...
    '編集中のファイルパス
    Dim _sFilePath As String = Nothing '起動時引数があればセット(issue#8)
...
#Region "ステップ画像・ヘッドレス実行"
    ...
    Public Function MakeImageFile(ByVal n As Integer, ByVal col As String, ByVal fpath As String, ByRef msg As String
        ...
        Dim calc As New clsCalcSquare(data, Me)
        ...
        Dim ret As Boolean = calc.CalcImage(stepImageData, isBackFace, chk底のみ.Checked, sasihimo)
    ...
    Public Function MakeImageFile2(ByVal n As Integer, ByVal col As String, ByVal fpath As String, ByVal isShow3D As Boolean, ByRef msg As String
        ...
        Dim stepModelImageData As New clsModelSquare(calc, Nothing)
        ...
        Try
            '存在チェック
            If Not IO.File.Exists(stepModelImageData.GifFilePath) Then
                Return False
            End If
            ...
            If isShow3D AndAlso radビューア.Checked Then
                stepModelImageData.ModelFileOpen(Nothing) '開く
            End If
            Return True

        Catch ex As Exception
            g_clsLog.LogException(ex, "MakeImageFile2")
            msg = ex.Message
            Return False
        End Try
    ...
    Public Function MakeImageFile(ByVal fpath As String, ByRef msg As String
        ...
        If Not _clsCalcSquare.CalcSize(CalcCategory.NewData, Nothing, Nothing) Then
            msg = _clsCalcSquare.p_sメッセージ
            Return False
        End If

        Dim imgData As New clsImageData(Nothing)
        Dim ret As Boolean = _clsCalcSquare.CalcImage(imgData, False, False, enum差しひも表示._底置き)
    ...
    Public Function MakeImageFile2(ByVal fpath As String, ByVal saveDir As String, ByRef msg As String
        ...
        Dim modelImageData As New clsModelSquare(_clsCalcSquare, Nothing)
...
#Region "プレビュー"
    ...
    Private ReadOnly Property sasihimo As enum差しひも表示
        Get
            '#95
            Dim is差しひも = ((0 < _clsDataTables.p_tbl差しひも.Count) AndAlso
                                Not chk底のみ.Checked AndAlso Not radうら.Checked)
            If is差しひも Then
                If rad回り込み.Checked Then
                    Return enum差しひも表示._回り込み
                ElseIf rad底置き.Checked Then
                    Return enum差しひも表示._底置き
                End If
            End If
            Return enum差しひも表示._非表示
        End Get
    End Property
